...
\begin{itemize}
\item \textbf{Small-item 下限保證（核心結果）：}baseline 為任何 EF1 allocation 至少達最優 NSW 的 $e^{-1/e}\approx0.692$ 倍；證明在 $\epsilon$-small-item condition（任一 good 價值 $\le\epsilon\mu$，$\mu$ 為平均份額）下可提升至 $\max\{e^{-1/e},\rho_n(\epsilon)\}$，其中 $\rho_n(\epsilon)=\min_{1\le k\le n-1}\big[(1-\tfrac{k\epsilon}{n})^{n-k}(1+\tfrac{(n-k)\epsilon}{n})^{k}\big]^{1/n}$，取自 EF1 所允許最不平均的 bundle 價值分佈。
\item \textbf{福利損失是 $\epsilon$ 的二次項：}$\rho_n(\epsilon)=1-\tfrac{1}{8}\epsilon^{2}+O(\epsilon^{3})=1-O(\epsilon^{2})$，$\epsilon\to0$ 時收斂至 1；最大物品佔比減半，福利損失只剩四分之一——物品夠小時 EF1 幾乎不犧牲 NSW。
